...
class InceptionI3dLSTM(snt.AbstractModule):
  ...
  def _build(self, inputs, is_training, dropout_keep_prob=1.0):
    ...
    with tf.variable_scope(end_point):
      ...
      layer ={ 'weights': tf.Variable(tf.random_normal([16, self._num_classes])),'bias': tf.Variable(tf.random_normal([self._num_classes]))}
